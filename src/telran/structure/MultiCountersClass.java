...
public class MultiCountersClass implements MultiCounters {
	...
	public Integer addItem(Object item) { // O[LogN]
		...
		if (!hashmap.containsKey(item)) {
			hashmap.put(item, 1);
			putFirstItemOccurrence(item);
		} else {
			int incrementValue = hashmap.get(item) + 1;
			hashmap.put(item, incrementValue);
			res = incrementValue;
			setFillingAndUpdating(item, incrementValue);
		}
		return res;
	}

	private void setFillingAndUpdating(Object item, int incrementValue) {
	...
	private void putFirstItemOccurrence(Object item) {
		if (treemap.isEmpty()) {
			HashSet<Object> set = new HashSet<>();
			set.add(item);
			treemap.put(1, set);
